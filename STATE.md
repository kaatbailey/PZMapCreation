...
| Repo | URL | Purpose |
|---|---|---|
| PZMapCreation | https://github.com/kaatbailey/PZMapCreation | Java pipeline, GIS import, furniture system, orchestrator |
| PZMapMaker | https://github.com/kaatbailey/PZMapMaker | C++ port (Qt6 Widgets + OpenGL 4.6) — front end decided, in progress |

Local clones live at `~/Documents/PZMapCreation` and `~/Documents/PZMapMaker`.

---

## 0a. Standing environment
...
# Render mod tiles (run after each generation — safe, takes seconds)
# NOTE: do NOT pass the mod name as a CLI argument — pzmap2dzi takes job names,
# not map names. The orchestrator registers the mod in conf first and runs:
cd ~/Documents/PZMapCreation/pzmap2dzi
rm -rf ~/Documents/PZMapCreation/map-output/html/map_data/mod_maps/PZGisImport
.venv/bin/python main.py -c conf/conf.yaml render base_top
...
## 0b. Standing rules (always follow)
...
- **Always give the exact fish command** for any build, run, scan, or tool
  invocation. Never describe — write it out ready to run.
- **Before chasing a subsidiary bug**, name the tradeoff and let the user decide.
- **Never create hosted artifacts (claude.ai pages).** All deliverables are
  files — sent as downloads or committed to the repo. Markdown for docs,
  source files for code. No HTML pages published to claude.ai.
- **Files go directly into `src/main/java/pzformat/`** — never copy from Downloads.
...
The pipeline now has a web UI. See `ORCHESTRATOR.md` in the repo root for
the full runbook. Quick start:
...
The orchestrator:
- Runs the same commands as STATE.md §0a, in order, without retyping
- Streams output to the browser in real time
- Registers the mod in pzmap2dzi's conf before rendering (fixes the wrong-map bug)
- Guards against silent generate failures (exit 0 with 0 cells)
- Preserves spawn points across regenerates
- Sends placement.html/modmap.html origin/spawns back automatically via POST

To switch to the C++ generator when PZMapMaker is ready:
```fish
...
Paste over `$GISMAP/spawnpoints.lua` — no regenerate needed. New game required.
Note: regenerating the mod OVERWRITES spawnpoints.lua with auto spawns.
The orchestrator preserves and restores spawn points automatically.
...
- **Always give exact fish commands** for builds/runs.
- **Never create hosted artifacts (claude.ai pages)** — all deliverables are
  files sent as downloads or committed to the repo.
- **Cell size = 256 tiles.** Scan scripts divide by 256.
- **`oven_ind` removed from TilePalette** — boilers/smokers gone from all rooms.
- **`stalls()` is the only correct way to place stall tiles** — do not replicate
  with raw `_02_*` indices via stampSet; they render as floating geometry.
...
- **AUTHORED strategy** used for commercial bathrooms only.
- **base render done once** (167 min). Never re-render base.
- **`render base_top` only** — never pass a mod name as a CLI argument to
  pzmap2dzi. The orchestrator registers the mod in conf first; manual runs
  use `main.py -c conf/conf.yaml render base_top` with no map name.
- **Java = laboratory, C++ = product.** Port after stability.
- **MAX_PITCH_SPAN = 12** — wider buildings get flat roofs until two-level roofs are measured.
- **spawnpoints.lua uses the 300-tile legacy grid**, never 256. modmap.html spawn picker uses /300 and %300.
- **Files go directly into `src/main/java/pzformat/`** — never copy from Downloads.
- **maxTiles = 0** — auto-sizes from area boundary. Never pass a hardcoded tile
  count; it will clip the map silently.
- **C++ front end decided: Qt6 Widgets + OpenGL 4.6** — no web UI, no Electron.
  See `C1_ARCHITECTURE.md §1.1` in PZMapMaker.
...
| `rooms.txt` | RoomScan input — room-type headers + coordinates |
| `map-output/html/placement.html` | Place mod on vanilla map (localhost:8880/placement.html) |
| `pzorch/` | Web orchestrator — see ORCHESTRATOR.md |
